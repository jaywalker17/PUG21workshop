...
```{r}
vect_characters <- c("hund", "katze", "mouse")
vect_decimal <- c(1.1, 1.3, 2.3, 3.4)
vect_integer <- c(1, 2, 6, 7, 10)
vect_logical <- c (FALSE, TRUE, FALSE)
fem_gender <- c(TRUE, FALSE, TRUE, TRUE)
...
```{r}
vect_kombi <- c(vect_decimal, vect_integer)
...
1.8) Can you multiply every other element of `c(2, 4, 6, 4, 9, 12)` with 2, but leave the others unchanged?
```{r}
...
summary(hotdog_2018)
summary(hotdog_2019)
summary(hotdog_genders)

2.5) Use `head()` and `tail()` on either of the vectors. What do these functions do?
  
# gives first or last 6 elements, if other number of elements desired, specify (see below)
```
...
#also possible
hotdog_length <- length(hotdog_names)
hotdog_names[hotdog_length]
...
```{r}
hotdog_male_2019 <- hotdog_2019[hotdog_genders=="male"]
hotdog_female_2018 <- hotdog_2018[hotdog_genders=="female"]
