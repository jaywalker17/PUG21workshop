...
nerd %>% select(age, gender, married, Q1:Q26)
```
...
nerd %>% select(voted, age, everything())
```
...
#also possible in filter function
nerd_red %>% 
  filter(gender==2, age < 30)
...
#another version, is age a part of this set using %in%
nerd_red %>% 
...
#more precise
nerd_red %>% 
  filter(gender==1 & (married==2 | married == 3))
...
#using table function, leaving tidyverse
...
nerd_red <- nerd_red %>% 
  filter(married %in% 1:3, age %in% 1:100)
...
#first sorts by gender, then sorted by age within that
...
nerd_red <- 
  nerd_red %>% 
  mutate(age_character = as.numeric(age_character), .after=age)
...
```{r}
nerd_red %>% 
  mutate(gender_corr=gender-1, .after=gender)
